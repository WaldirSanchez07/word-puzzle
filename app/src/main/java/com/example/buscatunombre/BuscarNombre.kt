...
import kotlinx.android.synthetic.main.activity_buscar_nombre.*
import java.util.*

class BuscarNombre : AppCompatActivity(), View.OnTouchListener {
    ...
    //
    var answers = arrayOf(Word("KOTLIN"))
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        generateBoardHelper(nombre.toString())
    ...
    fun addElement(arr: Array<Word>, element: String): Array<Word> {
        val mutableArray = arr.toMutableList()
        mutableArray.add(Word(element))
        return mutableArray.toTypedArray()
    }

    private fun generateBoardHelper(nombre: String){
        println("*** IMPRIMIR ***")
        println(nombre)
        answers = addElement(answers, nombre)
//        dictionary.plus(nombre to R.id.firstname)
        println(Arrays.toString(answers))
    ...
    private fun checkAnswer(start: String, end: String){
        ...
        if (isFound){
//            findViewById<TextView>(dictionary.getValue(foundWord)).setTextColor(resources.getColor(R.color.purple_500))
            Toast.makeText(applicationContext, "Palabra encontrada!", Toast.LENGTH_LONG).show()
        } else {
...
//     companion object {
//
//        val answers = arrayOf(Word("KOTLIN"))
//        const val letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
//        const val dim = 10
//        val dictionary = mapOf("KOTLIN" to R.id.kotlin)
//
